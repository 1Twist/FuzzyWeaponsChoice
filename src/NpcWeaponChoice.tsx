...
// --- SUA LÓGICA FUZZY ORIGINAL (100% INTOCADA) ---
...
type FuzzyChartProps = { data: any[]; lines: { key: string; color: string }[]; verticalLineX?: number; title: string; };
const FuzzyChart = ({ data, lines, verticalLineX, title }: FuzzyChartProps) => (
    <div>
        <h4 className="text-center font-semibold mb-2">{title}</h4>
        <ResponsiveContainer width="100%" height={180}>
            <LineChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255, 255, 255, 0.2)"/>
                <XAxis dataKey="x" type="number" domain={[0, 100]} tick={{ fill: 'white', fontSize: 12 }}/>
                <YAxis domain={[0, 1]} tick={{ fill: 'white', fontSize: 12 }}/>
...
// --- DEMAIS FUNÇÕES E DADOS NECESSÁRIOS PARA A UI ---

// O tipo `WeaponResult` foi estendido para incluir a função de agregação, necessária para os gráficos
...
export const NpcWeaponChoice = () => {
  ...
  // O estado agora usa o tipo estendido para armazenar os dados do gráfico
  ...
  // O `handleCalculate` foi adaptado para gerar a `aggregatedFn` que a UI precisa
  const handleCalculate = () => {
    ...
    const allResults = weaponsToEvaluate.map(weapon => {
      // Re-criamos a função aggregatedOutput aqui para passá-la para a UI
  ...
  const distanceChartData = generateChartData(fuzzifyDistance, ['Perto', 'Medio', 'Longe']);
  const ammoChartData = generateChartData(fuzzifyAmmo, ['Baixa', 'Media', 'Alta']);
          ...
          {results && (
            <div className="mt-8 space-y-4">
              {results.map((res) => {
                ...
                // Objeto para encontrar a função de avaliação correta para cada arma
                      ...
                      <div className="text-right pl-2">
                        <div className="text-xs text-gray-400">Score Fuzzy</div>
                          <Popover>
                            ...
                            <PopoverContent className="w-[500px] bg-gray-800/90 border-gray-700 text-white backdrop-blur-sm">
